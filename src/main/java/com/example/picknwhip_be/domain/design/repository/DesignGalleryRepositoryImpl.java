...
public class DesignGalleryRepositoryImpl implements DesignGalleryRepositoryCustom {
  ...
  public List<DesignResDTO.DesignNameDTO> fetchDesignNamesByShopId(Long shopId) {
    ...
    public Page<DesignResDTO.GalleryItemDTO> searchGallery(
        ...
        BooleanExpression isMyPickCondition = (userId != null)
        ...
        List<DesignResDTO.GalleryItemDTO> content = queryFactory
                ...
                .leftJoin(favorite).on(
                        favorite.designGallery.eq(design).and(isMyPickCondition)
                )
                .where(
    ...
    private OrderSpecifier[] getOrderSpecifiers(
            String sortType, QShop shop, QDesignGallery design, Double userLat, Double userLon, Long seed) {
        ...
        if ("NEARBY".equals(sortType) && userLat != null && userLon != null) {
            NumberTemplate<Double> distance = Expressions.numberTemplate(Double.class,
                    ...
                    shop.location, userLon, userLat);// {1}=userLon, {2}=userLat

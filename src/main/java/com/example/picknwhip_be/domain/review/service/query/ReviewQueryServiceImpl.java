...
public class ReviewQueryServiceImpl implements ReviewQueryService {
    ...
    public ReviewResDTO.BestReviewListDTO getBestCustomReviews() {
        ...
        List<ReviewSelectedKeyword> allKeywords = reviewSelectedKeywordRepository.findAllByReviewIdIn(reviewIds);
        List<Object[]> likeCountData = reviewLikeRepository.countByReviewIdIn(reviewIds);

        // 데이터 메모리 매핑
        Map<Long, List<OrderItem>> optionsByOrderId = allOrderItems.stream()
                .collect(Collectors.groupingBy(item -> item.getOrder().getId()));

        Map<Long, List<String>> keywordsByReviewId = allKeywords.stream()
                .collect(Collectors.groupingBy(
                        k -> k.getReview().getId(),
                        Collectors.mapping(k -> k.getKeyword().getLabel(), Collectors.toList())));

        Map<Long, Long> likeCounts = likeCountData.stream()
                .collect(Collectors.toMap(row -> (Long) row[0], row -> (Long) row[1]));

        // 아이템 DTO 리스트 생성
        List<ReviewResDTO.BestReviewItemDTO> items = reviews.stream()
                .map(review -> {
                    List<OrderItem> myOptions = optionsByOrderId.getOrDefault(review.getOrder().getId(), Collections.emptyList());
                    List<String> myKeywords = keywordsByReviewId.getOrDefault(review.getId(), Collections.emptyList());
        ...
        return ReviewResDTO.BestReviewListDTO.builder()
                .items(items)
                .build();
    }
    ...
    private ReviewResDTO.BestReviewItemDTO convertToBestReviewItemDTO(
        ...
        ReviewResDTO.CakeColorsDTO colorsDTO = ReviewResDTO.CakeColorsDTO.builder()
                .icingColor(icingColor)
                .sheetColor(sheetColor)
                .creamColor(creamColor)
                .build();

        ReviewResDTO.CakeOptionDTO optionDTO = ReviewResDTO.CakeOptionDTO.builder()
